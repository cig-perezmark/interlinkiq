...
function displayChart(data) {
    ...
    // Update the modal title with the Customer name
    ...
    if (auditData.length > 0) {
        am5.ready(function() {
            try {
                ...
                makeSeries("Audit Report", "audit_report_start", am5.color(0x7c6ed3), false, false, auditYAxis);
                makeSeries("Audit Report Duration", "audit_report_duration", am5.color(0xFFD700), true, true, auditYAxis);

                makeSeries("Audit Certificate", "audit_certificate_start", am5.color(0x0096FF), false, false, auditYAxis);
                makeSeries("Audit Certificate Duration", "audit_certificate_duration", am5.color(0x4A731C), true, true, auditYAxis);

                makeSeries("Audit Corrective Action", "audit_corrective_action_start", am5.color(0xd981cd), false, false, auditYAxis);
                makeSeries("Audit Corrective Action Duration", "audit_corrective_action_duration", am5.color(0x883407), true, true, auditYAxis);

                makeSeries("Reviewed", "reviewed_start", am5.color(0xFA8072), false, false, auditYAxis);
    ...
    series1.bullets.push(function() {
      return am5.Bullet.new(root1, {
        sprite: am5.Label.new(root1, {
          ...
          centerX: am5.p50,
          populateText: true,
          fill: am5.color(0xFFFFFF) // Setting text color to white
        })
      });
    });
    ...
    fetch('AnalyticsIQ/customer_send_data.php')
      ...
      .then(data => {
        var data1 = [
          ...
          { category: "Do Not Use", value: parseInt(data.donutData.total_send) - parseInt(data.donutData.pending) - parseInt(data.donutData.approved) - parseInt(data.donutData.non_approved) + parseInt(data.donutData.emergency_use_only), open: 0, columnConfig: { fill: am5.color(0xFFC000) }, displayValue: parseInt(data.donutData.do_not_use) }
        ];
