...
am5.ready(function () {
  ...
  function updateChart(field, data) {
    ...
    if (field === 'filled_out' || field === 'signed' || field === 'compliance') {
      chartData.push({
        category: fieldLabels[field][0] + ' (' + data[0].count_1 + ')',
        value: data[0].count_1
      });
      chartData.push({
        category: fieldLabels[field][1] + ' (' + data[0].count_0 + ')',
        value: data[0].count_0
      });
    } else if (field === 'frequency') {
      chartData.push({
        category: fieldLabels[field][0] + ' (' + data[0].daily + ')',
        value: data[0].daily
      });
      chartData.push({
        category: fieldLabels[field][1] + ' (' + data[0].weekly + ')',
        value: data[0].weekly
      });
      chartData.push({
        category: fieldLabels[field][2] + ' (' + data[0].monthly + ')',
        value: data[0].monthly
      });
      chartData.push({
        category: fieldLabels[field][3] + ' (' + data[0].quarterly + ')',
        value: data[0].quarterly
      });
      chartData.push({
        category: fieldLabels[field][4] + ' (' + data[0].biannual + ')',
        value: data[0].biannual
      });
      chartData.push({
        category: fieldLabels[field][5] + ' (' + data[0].annually + ')',
        value: data[0].annually
      });
    } else if (field === 'assigned_count') {
      data.forEach(function (row) {
        chartData.push({
          category: row.full_name + ' (' + row.assigned_count + ')',
...
function createCharts(data) {
  ...
  createDonutChart("filledOutChart", [{
    category: "Filled Out (".concat(data.filled_out.filled_out_1, ")"),
    value: parseInt(data.filled_out.filled_out_1),
    isNotFilledOut: false
  }, {
    category: "Not Filled Out (".concat(data.filled_out.filled_out_0, ")"),
    value: parseInt(data.filled_out.filled_out_0),
    isNotFilledOut: true
  }], "Filled Out Status"); // Create signed chart

  createDonutChart("signedChart", [{
    category: "Signed (".concat(data.signed.signed_1, ")"),
    value: parseInt(data.signed.signed_1),
    isNotSigned: false
  }, {
    category: "Not Signed (".concat(data.signed.signed_0, ")"),
    value: parseInt(data.signed.signed_0),
    isNotSigned: true
...
function createDonutChart(divId, data, title) {
  am5.ready(function () {
    console.log("Creating donut chart in ".concat(divId, " with data:"), data); // Log the data for each chart
    ...
    series.data.setAll(data); // Create legend
...
function createBarChart(divId, data, title) {
  am5.ready(function () {
    console.log("Creating bar chart in ".concat(divId, " with data:"), data); // Log the data for each chart
...
function createCurvedColumnChart(divId, data, title) {
  am5.ready(function () {
    console.log("Creating curved column chart in ".concat(divId, " with data:"), data); // Log the data for each chart
...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  fetch('AnalyticsIQ/assigned_to_data.php').then(function (response) {
    ...
    console.log("Chart Data:", data); // Check if data is empty
    ...
    am5.ready(function () {
      ...
      var series = chart.series.push(am5percent.PieSeries["new"](root, {
        ...
        innerRadius: 0 // innerRadius: am5.percent(50) // This makes it a donut chart
...
//     }
// }
//# sourceMappingURL=rvm_chart.dev.js.map
