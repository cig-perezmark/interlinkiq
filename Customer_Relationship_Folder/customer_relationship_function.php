...
if (isset($_POST['update_details_account'])) {
    ...
    $Account_Directory = (!empty($_POST['Account_Directory'])) ? mysqli_real_escape_string($conn, $_POST['Account_Directory']) : 0;
...
if (isset($_POST['btnnotes_submit'])) {  
    ...
    $sql = "INSERT INTO tbl_Customer_Relationship_Notes (Notes, crm_ids, user_cookies) VALUES ('$Notes', '$crm_ids', '$userID')";
    
    if (mysqli_query($conn, $sql)) {
        $last_insert_id = mysqli_insert_id($conn);
        $action = 'Added new Notes';
        $activity = "INSERT INTO tbl_crm_history_data (contact_id, user_id, performer_name, action_taken, type, action_id) VALUES ($crm_ids, $userID, '$name', '$action', 2, $last_insert_id)";
        ...
        echo "Error inserting notes: " . $conn->error;
    }
}
...
if (isset($_POST['update_notes'])) {  
    ...
    $sql = "UPDATE tbl_Customer_Relationship_Notes SET Notes = '$Notes' WHERE notes_id = '$ID'";
