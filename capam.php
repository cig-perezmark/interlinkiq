<?php
	$title = "Corrective Action and Preventive Action Management";
	$site = "capam";
	$breadcrumbs = '';
	$sub_breadcrumbs = '';
...
<style type="text/css">
	...
	@media print {
		...
		#tableDataOpen_wrapper,
		#tableDataClose_wrapper {
									...
									<div class="portlet light">
										<div class="portlet-body">
											<table class="table table-bordered table-hover" id="tableDataOpen">
												...
												<tbody>
													<?php
													$selectCamOpen = mysqli_query($conn, "SELECT c.* FROM tbl_cam AS c LEFT JOIN tbl_supplier AS s ON s.ID = c.related_supplier WHERE c.status = 0 AND (c.user_id = $switch_user_id OR s.ID IS NOT NULL ) ORDER BY c.ID DESC");
													if (mysqli_num_rows($selectCamOpen) > 0) {
														while ($rowOpen = mysqli_fetch_array($selectCamOpen)) {
															...
															if ($current_client == 0) {
																echo '<td>' . htmlentities($cam_reference) . '</td>';
															}
									...
									<div class="portlet light">
										<div class="portlet-body">
											<table class="table table-bordered table-hover" id="tableDataClose">
												...
												<tbody>
													<?php
													$selectCamOpen = mysqli_query($conn, "SELECT c.* FROM tbl_cam AS c LEFT JOIN tbl_supplier AS s ON s.ID = c.related_supplier WHERE c.status = 1 AND (c.user_id = $switch_user_id OR s.ID IS NOT NULL ) ORDER BY c.ID DESC");
													if (mysqli_num_rows($selectCamOpen) > 0) {
														while ($rowOpen = mysqli_fetch_array($selectCamOpen)) {
															$cam_ID = $rowOpen['ID'];
															$cam_reference = htmlentities($rowOpen['reference'] ?? '');
										...
										<div class="portlet light">
											<div class="portlet-body">
												<div class="row form-horizontal">
													...
													<div class="col-md-1">
														<select class="form-control margin-bottom-15" name="product_related" onchange="changeMulti(this, 1)">>
															<option value="1">Yes</option>
															<option value="0">No</option>
														</select>
													</div>
													<div class="<?php echo $switch_user_id == 1 ? '' : ''; ?>">
														<div class="product_related_1 col-md-2 col-md-offset-1">
															<select class="form-control" name="related_supplier">
																<option value="0">Select Supplier</option>
																<?php
																$selectSupplier = mysqli_query($conn, "SELECT ID, name FROM tbl_supplier WHERE user_id = $switch_user_id AND facility_switch = $facility_switch_user_id AND page = 1 AND is_deleted = 0 ORDER BY name");
														...
														<div class="product_related_1 col-md-2">
															<select class="form-control" name="related_product">
																...
																$selectProduct = mysqli_query($conn, "
																	...
																	*
																	FROM (
																		...
																		FROM tbl_products
																		WHERE user_id = $switch_user_id AND facility_switch = $facility_switch_user_id AND deleted = 0
																		...
																		WHERE s.user_id = $switch_user_id AND s.facility_switch = $facility_switch_user_id
																...
																if (mysqli_num_rows($selectProduct) > 0) {
																	while ($rowProduct = mysqli_fetch_array($selectProduct)) {
																		echo '<option value="' . $rowProduct['ID'] . '">' . htmlentities($rowProduct['name'] ?? '') . '</option>';
																	}
																}
																?>
															</select>
														...
														<div class="mt-checkbox-inline hide">';
															...
															if (mysqli_num_rows($selectCategory) > 0) {
																while ($rowCat = mysqli_fetch_array($selectCategory)) {
																	...
																	echo '<label class="mt-checkbox mt-checkbox-outline">
																		<input type="checkbox" name="category_ids[]" value="' . $cat_ID . '"> ' . $cat_name . '
																		<span></span>
																	</label>';
																}
															}
															...
															echo '<label class="mt-checkbox mt-checkbox-outline">
																<input type="checkbox" name="category_ids[]" value="0" onClick="btnCategory(this)"> Other
																<span></span>
															</label>
											...
											<div class="portlet light">
												<div class="portlet-body">
													<div class="form-group">
														...
														<div class="mt-checkbox-list">
															...
															<label class="mt-checkbox mt-checkbox-outline checkboxx">
																<input type="checkbox" class="status" name="status" value="2" onclick="highchart_status(1)" checked> Open
																<span></span>
															</label>
															<label class="mt-checkbox mt-checkbox-outline checkboxx">
													...
													<div class="form-group">
														...
														<div class="mt-radio-list navGeneral">
															<label class="mt-radio mt-radio-outline"> (All)
																<input type="radio" class="yearAll" name="year" onclick="highchart_year(1)" checked />
																<span></span>
															</label>
															<?php
																...
																if (mysqli_num_rows($selectYYYY) > 0) {
																	while ($rowYYYY = mysqli_fetch_array($selectYYYY)) {
																		...
																		echo '<label class="mt-radio mt-radio-outline"> ' . $data_YYYY . '
																			<input type="radio" class="year" name="year" value="' . $data_YYYY . '" onclick="highchart_year(1)" />
																			<span></span>
																		</label>';
																	}
																}
													...
													<div class="form-group navGeneralx">
														...
														<div class="mt-checkbox-list">
															<label class="mt-checkbox mt-checkbox-outline monthAllSel hide"> (All)
													...
													<div class="form-group">
														...
														<div class="mt-checkbox-list">
															<?php
																$selectSupplier = mysqli_query($conn, "
																	SELECT s.ID, s.name 
																	FROM tbl_supplier AS s
																...
																if (mysqli_num_rows($selectSupplier) > 0) {
																	while ($rowSupplier = mysqli_fetch_array($selectSupplier)) {
																		echo '<label class="mt-checkbox mt-checkbox-outline"> ' . htmlentities($rowSupplier['name'] ?? '') . '
																			<input type="checkbox" class="supplier" name="supplier" value="' . $rowSupplier['ID'] . '" onclick="highchart_supplier(1)" />
																			<span></span>
																		</label>';
																	}
																}
													...
													<div class="form-group">
														...
														<div class="mt-checkbox-list">
															<?php
																...
																if (mysqli_num_rows($selectProduct) > 0) {
																	while ($rowProduct = mysqli_fetch_array($selectProduct)) {
																		echo '<label class="mt-checkbox mt-checkbox-outline"> ' . htmlentities($rowProduct['name'] ?? '') . '
																			<input type="checkbox" class="product" name="product" value="' . $rowProduct['ID'] . '" onclick="highchart_product(1)" />
																			<span></span>
																		</label>';
																	}
																}
		...
		<script src="staging-dev/capam/script.js"></script>
		...
		<script type="text/javascript">
			...
			$('#jstree_category').on('changed.jstree', function(e, data) {
				...
				$('#category_id').val(idString);
			});

			function createJSTree() {
				$.ajax({
					type: 'GET',
					url: 'function.php?jstree_category=' + 1,
					dataType: "html",
					success: function(data) {
						jsObject = JSON.parse(data);

						$('#jstree_category').on('ready.jstree', function() {
							$('#jstree_category').jstree('open_all');
						}).jstree({
							"core": {
								"data": jsObject
							},
			...
			function widget_jstree(data, n) {
				// alert(data);
				$('#jstree_category_' + n).on('ready.jstree', function() {
					$('#jstree_category_' + n).jstree('open_all');
				}).jstree({
					"core": {
						"data": jsObject
					},
				...
				$('#jstree_category_' + n).on("changed.jstree", function(e, data) {
					var selectedIds = data.selected;
					$('#category_id_' + n).val(selectedIds.join(", "));
				});
			}

			function widget_jstree_select(n) {
				$('#jstree_category_' + n).on("ready.jstree", function() {
					var selectedIds = $('#category_id_tmp_' + n).val().split(", "); // Convert string to array
					selectedIds.forEach(function(id) {
						$('#jstree_category_' + n).jstree("check_node", id);
					});
				});
			}

			function changeMulti(e, modal) {
				$('.product_related_' + modal).toggleClass('hide')
			}
			...
			function btnEdit(id) {
				$.ajax({
					...
					success: function(data) {
						...
						widget_jstree(jsObject, 1);
						widget_jstree_select(1);
					}
				});
			}
				...
				$.ajax({
					...
					success: function(response) {
						if ($.trim(response)) {
							...
							$('#tableDataOpen').DataTable().destroy();
							...
							$('#tableDataOpen').DataTable().draw('page');
						} else {
							msg = "Error!"
						}
						l.stop();
			...
			function highchart_dashboard() {
				...
				suppliers = highchart_supplier(0);
				products = highchart_product(0);
				console.log("val " + val);
				...
				Highcharts.getJSON(
					'function.php?modalTrend_CAM=' + val + '&years=' + years + '&months=' + months + '&complaints=' + complaints + '&departments=' + departments + '&suppliers=' + suppliers + '&products=' + products,
					function(data) {
			...
			function highchart_supplier(id) {
				if (id == 1) {
					highchart_refresh()
				} else {
					var supplierTotal = $('.supplier').length;
					var supplierChecked = $('.supplier:checked').length;
					var suppliers = $('.supplier:checked').map(function(_, el) {
						return $(el).val();
					}).get();
					return suppliers;
				}
			...
			function highchart_product(id) {
				if (id == 1) {
					highchart_refresh()
				} else {
					var productTotal = $('.product').length;
					var productChecked = $('.product:checked').length;
					var products = $('.product:checked').map(function(_, el) {
						return $(el).val();
					}).get();
					return products;
				}
