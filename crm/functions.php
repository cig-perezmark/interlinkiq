<?php
    ...
    if (isset($_POST['manage_contactssss'])) {
        $selectedIds = $_POST['ids'];
        $action = $_POST['action'];
        $updated_by = $_COOKIE['first_name'] . ' ' . $_COOKIE['last_name'];
        ...
        // Check if all selected IDs exist in the database
        $missingIds = array_diff($selectedIds, array_column($records, 'crm_id'));
        ...
        if ($action == 'archive') {
            ...
            $action_taken = isset($_POST['taken_action']) ? htmlspecialchars($_POST['taken_action']) : '';
            $campaign_name = isset($_POST['name']) ? htmlspecialchars($_POST['name']) : '';
            ...
            foreach ($records as $data) {
                ...
                if ($stmt) {
                    ...
                    if (!mysqli_stmt_execute($stmt)) {
                        ...
                        $response = array('status' => 'error', 'message' => 'Error executing statement.');
    ...
    if (isset($_POST['upload_multiple_contacts'])) {
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            if (in_array($fileExtension, $fileExtensions)) {
                ...
                echo json_encode(['message' => 'Please select a valid CSV file.']);
                exit;
            }
        } else {
            echo json_encode(['message' => 'Please select a file.']);
            exit;
        }
    
        // Prepare the statement to check if the email exists
        $checkEmailStmt = mysqli_prepare($conn, "SELECT COUNT(*) FROM tbl_Customer_Relationship WHERE account_email = ? AND enterprise_id = ?");
        mysqli_stmt_bind_param($checkEmailStmt, "si", $checkEmail, $enterprise_id);
    
        // Check for errors in preparation
        if (!$checkEmailStmt) {
            echo json_encode(['message' => 'Error in preparing email check statement: ' . mysqli_error($conn)]);
            exit;
        }
    
        // Prepare the statement for insertion
        $stmt = mysqli_prepare($conn, "INSERT INTO tbl_Customer_Relationship(account_rep, enterprise_id, account_name, account_email, account_phone, account_fax, account_address, account_website, account_facebook, account_twitter, account_linkedin, account_interlink, account_product, account_service, account_certification, userID, Account_Source, mutiple_added, crm_date_added) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
        // Check for errors in preparation
        if (!$stmt) {
            echo json_encode(['message' => 'Error in preparing statement: ' . mysqli_error($conn)]);
            exit;
        }
    
        $failedEmailEntries = array();
        $otherFailedEntries = array();
        ...
        while (($getData = fgetcsv($csvFile)) !== false) {
            ...
            mysqli_stmt_bind_result($checkEmailStmt, $checkResult);
            mysqli_stmt_fetch($checkEmailStmt);
    
            if ($checkResult > 0) {
                // Email already exists, add to the array for existing emails
                $failedEmailEntries[] = $getData;
                continue;
            }
            // Execute the statement to insert if the email doesn't exist
            mysqli_stmt_execute($stmt);
    
            // Check if the statement executed successfully
            // if (mysqli_stmt_affected_rows($stmt) > 0) {
            //     // Add the details of the successfully inserted entry to the array
            //     $successfulEntries[] = $getData;
            // } else {
            //     // Add the details of the failed entry to the array for other errors
            //     $otherFailedEntries[] = $getData;
            // }
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                // Add the details of the successfully inserted entry to the array
                $successfulEntries[] = $getData;
                ...
                // Retrieve and include MySQL error message
                $errorCode = mysqli_errno($conn);
                $errorMessage = mysqli_error($conn);
                echo json_encode(['message' => 'Failed to insert data. Error code: ' . $errorCode . '. Error message: ' . $errorMessage]);
                exit;
            }
        }
        ...
        $existCount = 0;
        $failedCount = 0;
        $existEntries = ''; // Initialize a string to store existing entries
        $failedEntries = ''; // Initialize a string to store failed entries
    
        if (!empty($failedEmailEntries)) {
            foreach ($failedEmailEntries as $existData) {
                $existCount++;
                $existEntries .= '
                    <tr>
                        <td>' . $existCount . '</td>
        ...
        if (!empty($otherFailedEntries)) {
            foreach ($otherFailedEntries as $failedData) {
                $failedCount++;
                $failedEntries .= '
                    <tr>
                        <td>' . $failedCount . '</td>
        ...
        if (empty($existEntries) && empty($failedEntries)) {
            $response = 'success|All contact entries added successfully|' . $failedEntries . '|' . $existEntries;
        } else {
            $response = 'error|Some entries failed to save to the record because the entries exist in the record or have issues with special characters|' . $failedEntries . '|' . $existEntries;
        }
