<?php 
    ...
    $sql_count = "SELECT
        ...
        FROM new_emp_equipment_frequency WHERE owner_id = $switch_user_id";
...
<script src="https://cdn.canvasjs.com/canvasjs.min.js"></script>
                    <div class="row">
                        <div class="col-md-12" style="margin-bottom:20px;display:flex;justify-content:flex-end">
                            <div>
                                <a href="emp_bgl_zones" target="_blank" class="btn btn-primary">List of Areas</a>
                                <a class="btn btn-circle btn-icon-only btn-default" href="https://interlinkiq.com/emp_bgl_filter.php" target="_blank"><i class="icon-calendar"></i></a>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                                                    ...
                                                    <div class="scroller" style="height: 130px;" data-always-visible="1" data-rail-visible1="0" data-handle-color="#D7DCE2">
                                                        ...
                                                        <ul class="task-list">
                                                            <?php
                                                                ...
                                                                $frequency_sql = "SELECT * FROM new_emp_equipment_frequency WHERE owner_id = $switch_user_id AND day_of_the_week = $currentDayName AND frequency = 'Weekly' OR frequency = 'Daily' AND owner_id = $switch_user_id;";
                    ...
                    <div class="row" style="margin-top:20px" id="row_content">
                        <?php
                        $select_registered = "SELECT * FROM equipment_reg WHERE location = 'Packagings' AND enterprise_owner = $switch_user_id";
                                ...
                                <div class="portlet-body">
                                    <div class="table-scrollable">
                                                        ...
                                                        <div class="portlet light ">
                                                            <div class="portlet-title">
                                                                ...
                                                                <div class="actions">
                                                                    ...
                                                                    <a class="btn btn-circle btn-icon-only btn-default" data-toggle="modal" data-target="#modal_report" class="icon-wrench"><i class="icon-wrench"></i></a>
                                                                    <a class="btn btn-circle btn-icon-only btn-default" href="emp_equipment_trend.php?equip_id=<?= $equipmentId ?>" target="_blank"><i class="fa fa-arrows-v"></i></a>
                                                   ...
                                                   <script>
                                                        ...
                                                        $records_query = "SELECT parameter_name, n.id, n.parameter_id, n.equip_id, n.result AS result FROM new_emp_records n 
                                                                          INNER JOIN (
                                                                              SELECT parameter_id, MAX(id) AS max_id 
                                                                              FROM new_emp_records 
                                                                              WHERE equip_id = $equipmentId 
                                                                              GROUP BY parameter_id
                                                                          ) max_ids ON n.parameter_id = max_ids.parameter_id AND n.id = max_ids.max_id 
                                                        ...
                                                        // Fetch uom values for each equipment ID
                                                        $uom_values = [];
                                                        ...
                                                        while ($record = mysqli_fetch_assoc($records_result)) {
                                                            ...
                                                            // Fetch uom values for the current equipment ID if not already fetched
                                                            if (!isset($uom_values[$equip_id])) {
                                                                $select_settings = "SELECT equip_id, oum FROM new_emp_equipment_settings WHERE equip_id = $equip_id";
                                                                $select_result = mysqli_query($emp_connection, $select_settings);
                                                    
                                                                // Fetch all uom values for the current equipment ID
                                                                $uom_values[$equip_id] = [];
                                                                while ($select_assoc = mysqli_fetch_assoc($select_result)) {
                                                                    $uom_values[$equip_id][] = $select_assoc['oum'];
                                                                }
                                                            }
                                                    
                                                            // Use the uom values for the current equipment ID
                                                            $uom = implode(', ', $uom_values[$equip_id]);
                                                            
                                                            if (!$result) {
                                                                $result = 0;
                                                            }
                                                        ?>
                                                            dataPoints<?= $equipmentId ?>.push({ label: "<?= $parameter_name ?>", y: <?= $result ?>, uom: "<?= $uom ?>" });
                                                        <?php
                                                        }
                                                        ?>
                                                        ...
                                                        var chart<?= $equipmentId ?> = new CanvasJS.Chart("<?= $chartContainerId ?>", {
                                                            ...
                                                            data: [{
                                                                ...
                                                                toolTipContent: "{label}: {y} {uom}", // Display parameter name, y value, ands uom in the tooltip
                                                            }]
                                                        });
            ...
            <thead>
                <tr>
                    <?php 
                        ...
                        $report_sql = "SELECT DISTINCT sampled_date FROM `new_emp_records` WHERE `equip_id` = 121";
                        ...
                        $report_sql = "SELECT DISTINCT new_emp_records.parameter_id,new_emp_parameters.id, new_emp_parameters.parameter_name 
                                       ...
                                       WHERE `new_emp_records`.`equip_id` = 121";
                        $report_result = mysqli_query($emp_connection, $report_sql);
                        ...
                        foreach ($unique_dates as $date) {
                            foreach ($unique_parameters as $param) {
                                ...
                                $attendance_sql = "SELECT result, test_status FROM `new_emp_records` WHERE `equip_id` = 121 AND `sampled_date` = '$date' AND `parameter_id` = '$parameter_id'";
                ...
                <tbody>
                    <?php foreach ($unique_parameters as $param) : ?>
                        <tr>
                            ...
                            <?php foreach ($unique_dates as $date) : ?>
                                ...
                                ?>
                                <td style="color:<?= $color ?>;font-weight:500"><?= $attendance_value ?></td>
                            <?php endforeach; ?>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
