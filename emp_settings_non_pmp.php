...
?>
                ...
                <div class="portlet-body">
                    ...
                    <div class="portlet-title tabbable-line" style="display:flex;justify-content:space-between">
                        <ul class="nav nav-tabs">
                            <!--Emjay starts here-->
                            <li>
                                <a href="#forms" data-toggle="tab">Settings - <button class="btn btn-primary" data-toggle="modal" data-target="#add_micro">Add Parameter</button>&nbsp;&nbsp;<button class="btn btn-primary" data-toggle="modal" data-target="#add_equipment">Add Equipment</button></a>
                            </li>
                    ...
                    <div class="tab-content">
                        <div class="row" style="margin-top:25px" id="micro_div">
                            <div class="col-md-12" style="padding:10px">
                                <label class="caption-subject font-dark bold uppercase">Packaging - List of Equipments</label><br>
                                <div id="data_container">
                                    <?php
                                        $select_registered = "SELECT * FROM new_emp_equipment_register WHERE location = '{$_GET['area_name']}' AND enterprise_owner = 1";
                                            ...
                                            if ($registered_result) {
                                                foreach ($registered_result as $registered_result_row) {
                                                    // Explode the valuess from equipment_registered using a comma as the delimiter
                                                        $sql_frequency = "SELECT * FROM new_emp_equipment_frequency WHERE equipment_id = {$registered_result_row['equip_id']}";
                                                            ...
                                                            ?>
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-md-6" style="margin-top:25px;border:solid 1px #999;padding:10px;min-height:300px ">
                                                                        ...
                                                                        <label style="margin-top:5px">File Attachment <span class="add-file" data-id="<?= $registered_result_row['equip_id'] ?>">+</span></label>
                                                                        <div id="file-attachments<?= $registered_result_row['equip_id'] ?>" style="width:50%">
                                                                        ...
                                                                        <div id="additionalFields<?= $registered_result_row['equip_id'] ?>" <?php if ($sql_frequency_result_assoc['frequency'] == 'Weekly') echo 'style="display: block"'; ?>>
                                                                            <br>
                                                                        ...
                                                                        <div class="md-checkbox-inline" style="padding:15px;display:flow">
                                                                            ...
                                                                            <?php 
                                                                                $i = 0;
                                                                                $check_sql = "SELECT * FROM new_emp_parameters WHERE owner_id = 3 ";
                                                                                $check_result = mysqli_query($emp_connection, $check_sql);
                                                                                foreach($check_result as $check_row):
                                                                                    $sql_check = "SELECT * FROM new_emp_equipment_settings WHERE parameter_id = {$check_row['id']} AND equip_id = {$registered_result_row['equip_id']}";
                                                                            ...
                                                                            <div class="md-checkbox">
                                                                                ...
                                                                                <label  for="checkbox<?= $i .''. $registered_result_row['equip_id'] ?>">
                                                                                    <span></span>
                        ...
                        <div class="row">
                            <div class="col-md-12" style="display:flex;justify-content:flex-end;">
                                <button class="btn btn-primary" name="test_post" style="margin-right:30px">Save</button>
                            </div>
                        </div>
                    </div>
                    </form>
    ...
    <!-- Modal -->
    <div class="modal fade" id="add_equipment" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
...
<script>
    document.getElementById('save_micro').addEventListener('click', function () {
        ...
        xhr.send('action=save_micro&parameter_name=' + parameterName+'&owner_id='+<?= $switch_user_id ?>);
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php foreach($registered_result as $registered_result_row): 
            ...
            foreach ($check_result as $check_row): 
                $sql_check = "SELECT * FROM new_emp_equipment_settings WHERE parameter_id = {$check_row['id']} AND equip_id = {$equipment_id}";
                $sql_cehck_result = mysqli_query($emp_connection,$sql_check);
                $sql_check_result_assoc = mysqli_fetch_assoc($sql_cehck_result);
                // Check if the query result is not null
                if ($sql_check_result_assoc !== null) {
    ?>
                    ...
                    if (checkbox<?= $i ?> && checkbox<?= $i ?>.checked) {
                        toggleInputBoxes(parameterName<?= $i ?>, checkboxIndex<?= $i ?>, equipment_id<?= $i ?>, 'not_checked', minimum_input<?= $i ?>, maximum_input<?= $i ?>);
                    }
    <?php }
                $i++;
            endforeach;
...
function toggleInputBoxes(parameterName, checkboxIndex, equipment_id,status, minimum_input, maximum_input) {
    ...
    if (checkbox.checked) {
        ...
        // Create "cfu" option
        const cfuOption = document.createElement('option');
        cfuOption.value = 'cfu';
        cfuOption.textContent = 'cfu';
        selectUom.appendChild(cfuOption);
