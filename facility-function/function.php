<?php 
    ...
    if( isset($_POST['btnFunction']) ) {
        $name = htmlentities($_POST['name'] ?? '');
        $sql = "INSERT INTO tblFacilityDetails_Function(user_id, portal_user, name) VALUES('$user_id', '$portal_user', '$name')";
        if(mysqli_query($conn, $sql)){
            $last_id = mysqli_insert_id($conn);
            
            echo '<li>
                <label class="mt-checkbox mt-checkbox-outline">
                    <input type="checkbox" name="Facilty_Functions[]" value="'.$last_id.'" > '.$name.'
                    <span></span>
                </label>
            </li>';
         }
    }
    if( isset($_POST['btnQS']) ) {
        $name = htmlentities($_POST['name'] ?? '');
        $sql = "INSERT INTO tblFacilityDetails_QS(user_id, portal_user, name) VALUES('$user_id', '$portal_user', '$name')";
        if(mysqli_query($conn, $sql)){
            $last_id = mysqli_insert_id($conn);
            
            echo '<li>
                <label class="mt-checkbox mt-checkbox-outline">
                    <input type="checkbox" name="Quality_System[]" value="'.$last_id.'" > '.$name.'
                    <span></span>
                </label>
            </li>';
         }
