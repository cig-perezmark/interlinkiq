$(()=>{
  ...
  const colors_arr = [DRAFTED_COLOR, FOR_APPROVAL_COLOR, FOR_DELIVERY_COLOR, RECEIVED_COLOR, CANCELLED_COLOR]
  const all_status = ["Drafted", "For Approval", "For Delivery", "Received", "Cancelled"]
  ...
  function supply_data(modal_id, arr_string, exception_, data, check_for_disabled){
  ...
  function sig_div(index, name, label, loggedInUserName){
      ...
      return `<div id="showD${index}" class="signature${index} text-center mt-2">
              ...
              <div class="form-group mt12">
                ...
                <div class="col-lg-7">
                  <input type="text" class="form-control" name="${name}_name" value="${loggedInUserName}">
                </div>
              </div>
              <div class="form-group mt12">
  ...
  function display_status(data){
    var i
    switch(data){
      case 'drafted'      : i=0; break;
      case 'for_approval' : i=1; break;
      case 'for_delivery' : i=2; break;
      case 'received'     : i=3; break;  
      case 'cancelled'    : i=4; break;      
    }
  ...
  function display_comment_btn(total_comments, comment_type){
    return `<input type=button data-type="${comment_type || 'po'}" class="total_comment_btn btn btn-primary btn-sm" value=${total_comments}>`
  ...
  // ------ PURCHASES ------ 
  var all_purchases_main_table, draft_main_table, for_approval_main_table, for_delivery_main_table, received_main_table, cancelled_main_table
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, row) {
            ...
            div += delete_po_btn()
            div += sign_po_btn(row.id,'for_approval','drafted_by','Push to : For Approval', 1)
            div += print_po_btn()
            return div
          }
        }
    ...
    for_approval_main_table = $("#for_approval .main_table").DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          targets: 6,
          render: function (data, type, row) {
            return display_name(row.drafted_by_name, row.drafted_by_position, row.date_drafted)
          },
          width : "15%"
        },
        {
        ...
        },
        {
          ...
          render: function (data, type, row) {
            ...
            div += sign_po_btn(row.id,'for_delivery','approved_by','Push to : For Delivery', 3)
            div += print_po_btn()
            return div
          },
          width : "35%"
    ...
    for_delivery_main_table = $("#for_delivery .main_table").DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          targets: 6,
          render: function (data, type, row) {
            return display_name(row.approved_by_name, row.approved_by_position, row.date_approved)
          },
          width : "15%"
        },
        {
    ...
    received_main_table = $("#received .main_table").DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          targets: 6,
          render: function (data, type, row) {
            return display_name(row.po_received_by_name, row.po_received_by_position, row.date_po_received)
          },
          width : "15%"
        },
        {
    ...
    cancelled_main_table = $("#cancelled .main_table").DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          targets: 6,
          render: function (data, type, row) {
            return display_name(row.cancelled_by_name, row.cancelled_by_position, row.date_cancelled)
          },
          width : "15%"
        },
        {
  ...
  function generate_barchart(){
    ...
    var drafted_total = 0, for_approval_total = 0, for_delivery_total = 0, received = 0, cancelled = 0
    ...
    series_data = [];
    var all_totals = [drafted_total, for_approval_total, for_delivery_total, received, cancelled]
  ...
  $(document).on("click", ".reply_btn", function(){
    ...
    if($this.hasClass('active_reply')){
      ...
      $(comments_modal+" .submit").val("Add Notes").removeClass('btn-danger').addClass('btn-primary')
      $this.removeClass('red').addClass('green')
    }else{
      $this.addClass('active_reply').val("Replying..")
      $(comments_modal+" [name='parent_id']").val($this.attr('data-id'))
      $(comments_modal+" .submit").val("Add Reply").removeClass('btn-primary').addClass('btn-danger')
    ...
    $.ajax({
        ...
        success: function (response) {
          if(response == "error"){
            ...
            if(parent_id){ // if input is a reply
              $("#comments_area > .portlet-body > .timeline > .timeline-item").each(function(){
              ...
              $("#comments_modal [name='parent_id']").val("")
              $("#comments_modal .submit").val("Add Notes").removeClass('btn-danger').addClass('btn-primary')
              $(".reply_btn").val("Reply").removeClass('active_reply').removeClass('red').addClass('green')
            }else{
  ...
  $(document).on("click", ".push_po", function(){
    ...
    var title_html = `<strong class='${prefix == "cancelled_by" ? "text-danger" : "text-primary"}'>${modal_title}</strong>`
    ...
    $.ajax({
      ...
      dataType: "json",
      data: {
        get_login_details_name   : true,
      },
      success: function (response) {
        var loggedInUserName = response.user_logged;
        $("#signature_modal .modal-body .contents").html(sig_div(sig_counter, prefix, "Name", loggedInUserName));
  ...
  $(document).on("click", ".delete_po", function(){
    ...
    confirm_action(`Delete ${formdata.po}`, `Are you sure you want to delete this PO?`, function(){
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        response.materials.forEach(el=>{
          ...
          div += `<td><input type=text class=form-control name=sku value='${el.material_id}' readonly></td>`
          div += `<td><input type=text class=form-control name=item value='${el.material_name}' readonly></td>`
          div += `<td><input type=text class=form-control name=category value='${response.name}' readonly></td>`
          div += `<td><input type=number class=form-control name=quantity value=1></td>`
          div += `<td><input type=text class=form-control name=uom value='${el.material_uom}' readonly></td>`
          div += `<td><input type=text class=form-control name=price_per_unit value='${el.material_ppu}' readonly></td>`
          div += `<td><span class=display_currency></span></td>`
          div += `<td><input type=text class=form-control name=total_price value='${el.material_ppu}' readonly></td>`
          div += `<td><span class=display_currency></span></td>`
          div += `<td><input type=text class=form-control name=tax></td>`
          div += `<td><input type=hidden class=form-control name=category_id value='${response.category_id}'></td>`
          div += `</tr>`
        })
  ...
  $(document).on("click", "#add_po_modal .submit", function(){
    ...
    // confirm_action("Add Purchase Order", "Are you sure you want to add purchase order?", function(){
      ...
      var category_id      = $(`${modal_id} [name='category_id']`).val()         

      if(!supplier_id || !location_id || !expected_arrival || !created_date || !po || !currency){
        alert('Please enter supplier, location, expected arrival, PO number, currency')
        return
      }
      ...
      po_form.push(get_obj('category_id', category_id))
      ...
      po_form.push(get_obj("shipping", $("#shipping").val() || null ))
      
      var items_form = [];
      $("#add_po_modal .item_table tbody tr").each(function(){
        var parent = $(this)
        var is_checked = parent.find("input[type='checkbox']").is(":checked")
        const names = ["raw_material_id","item","sku","category","quantity","uom","price_per_unit","total_price","tax"]
        if(is_checked){
          var row_item = []
          names.forEach(name=>{
  ...
  function view_po(formdata, editable){
    ...
    if(editable == false){
      $(modal_id+" [name='expected_arrival'], "+modal_id+" [name='remarks']").prop('disabled','disabled')
      $(modal_id+" .submit").hide()
    }else{
      $(modal_id+" [name='expected_arrival'], "+modal_id+" [name='remarks']").removeAttr('disabled')
  ...
  $(document).on("change", "#add_po_modal .item_table [name='quantity']", function(){
    multiply_price(this)
    compute_total()
  })

  $(document).on("change", "#add_po_modal .item_table input[type='checkbox']", function(){
    compute_total()
  })

  $(document).on("keyup", "#add_po_modal .item_table [name='quantity'], #add_po_modal .item_table [name='price_per_unit']", function(){
    multiply_price(this)
    compute_total()
  })
  ...
  // ------ WAREHOUSE ------ 
  var warehouse_receiving_table,materials_inventory_table,stock_transfer_out_table,stock_transfer_in_table
  ...
  function warehouse_init(){
          ...
          },
          {
            ...
            render : function(data, type, row){
              if(row.record_type == 1){
                var span_color
                switch(data){
                  case "For Inspection" : span_color = "text-danger"; break;
                  case "Inspected"      : span_color = "text-warning"; break;
                  case "Completed"      : span_color = "text-success"; break;
                  default : span_color = "text-primary";
                }
     ...
     materials_inventory_table = $("#materials_inventory .main_table").DataTable({
        ...
        columns: [
          ...
          {
              title: "Price Per Unit", //7
              data: "price_per_unit",
          },
          {
              title: "Total Amount", //8
              // data: "total_amount",
          },
          {
              title: "Quantity", //9
          ...
          {
              title: "Stock Transfer In", //10
              data: "str_in",
          },
          {
              title: "Stock Transfer Out", //11
              data: "str_out",
          },
          {
              title: "Variance", //12
          ...
          },
          {
            ...
            render : function(data, type, row){
              return `<div class="cell_item text-center">
                <a href="#" class="cell_value ${data ? 'text-primary' : 'text-warning'}" data-column='reorder'>${data ? data : "Set"}</a>
              </div>`              
            }
          },
          {
        ...
        initComplete: function(){
          $("#materials_inventory div.dataTables_filter").prepend(location_elem);      
        },
        language : {
          search : "",
          searchPlaceholder : "Search"
  ...
  $(document).on('click', '.whouse_view_po_modal', function(e){
    ...
    supply_data(modal_id, "id,po_id,supplier_name,location,po,expected_arrival", "id,po_id,supplier_name,location,po,expected_arrival", data)
    ...
    if(data.date_po_received){
      ...
      $(`${modal_id} [name='remarks'], ${modal_id} [name='expected_arrival']`).attr('disabled', 'disabled')

      var sig_div1 = get_fsig("Received By", data.po_received_by_sig, data.po_received_by_name, data.po_received_by_position, data.date_po_received)
      $("#whouse_received_container").html(sig_div1)
      $(`${modal_id} .submit`).addClass('hidden') 
    }else{
      $(`${modal_id} [name='remarks'], ${modal_id} [name='expected_arrival']`).removeAttr('disabled')

      $("#whouse_received_container").html(sig_div(4,"po_received_by","Received By"))
    ...
    $.ajax({
        ...
        success: function (response) {
          ...
          response.forEach(el=>{
            var div = `<tr id=${el.id}>
            <td><input type=text class=form-control value='${el.sku}' disabled></td>
            <td><input type=text class=form-control value='${el.item}' disabled></td>
            <td><input type=text class=form-control value='${el.category}' disabled></td>
            <td><input type=text class=form-control value='${el.quantity}' disabled></td>
            <td><input type=text class=form-control value='${el.uom}' disabled></td>
            <td><input type=text class=form-control name=quantity_received value="${el.quantity_received ? el.quantity_received : ''}" ${el.quantity_received ? 'disabled' : ''}></td>`
  ...
  $(document).on('click', "#view_warehouse_receiving_po_modal .submit", function(){
    ...
    // confirm_action("Save Entries", "Are you sure you want to save entries?", function(){
      ...
      form_items.push(get_obj('remarks', $(`${modal_id} [name='remarks']`).val()))

      var po_received_by_name = $(`${modal_id} [name='po_received_by_name']`).val()
      if(po_received_by_name){
        po_received_by_name = po_received_by_name.trim()
        form_items.push(get_obj('po_received_by_sig', $(`${modal_id} [name='po_received_by_sig']`).val()))
        form_items.push(get_obj('po_received_by_name', po_received_by_name))
        form_items.push(get_obj('po_received_by_position', $(`${modal_id} [name='po_received_by_position']`).val()))
      }else{
  ...
  $(document).on('click', '.whouse_view_supplier_modal', function(e){
    ...
    var data = $("#receiving table").dataTable().fnGetData(tr);
    // console.log(data)
    supply_data(modal_id, "id,date_received,arrival_time,po,supplier_name,invoice,trailer_no,trailer_plate,trailer_seal", "po,supplier_name", data, data.date_supplier_verified)
    $(modal_id+" [name='warehouse_receiving_id']").val(data.id)

    if(data.date_supplier_inspected){
      var sig_div1 = get_fsig("Inspected By", data.supplier_inspected_by_sig, data.supplier_inspected_by_name, data.supplier_inspected_by_position, data.date_supplier_inspected)
      $("#inspected2_container").html(sig_div1)
    }else{
      $("#inspected2_container").html(sig_div(6,"supplier_inspected_by","Inspected By"))
      $('#signature-pad-'+6).jSignature({'width': 300, 'height':50});
    }

    if(data.date_supplier_verified){
      var sig_div2 = get_fsig("Verified By", data.supplier_verified_by_sig, data.supplier_verified_by_name, data.supplier_verified_by_position, data.date_supplier_verified)
      $("#verified2_container").html(sig_div2)

      // hide submit button
      $(`${modal_id} .submit`).addClass('hidden')
      $("#warehouse_receiving_table1 tfoot,#warehouse_receiving_table2 tfoot").addClass("hidden")

    }else{

      $("#verified2_container").html(sig_div(7,"supplier_verified_by","Verified By"))
      $('#signature-pad-'+7).jSignature({'width': 300, 'height':50});
      
      $(`${modal_id} .submit`).removeClass('hidden')
      $("#warehouse_receiving_table1 tfoot,#warehouse_receiving_table2 tfoot").removeClass("hidden")
    }
    // console.log(data)
            ...
            var div = `<tr id=${el.id}>
              ...
              <td class=text-center colspan=3>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${index+el.id}" value=1 ${el.value==1 ? 'checked' : ''} ${data.date_supplier_verified ? 'disabled' : ''}> Yes
                </label>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${index+el.id}" value=2 ${el.value==2 ? 'checked' : ''} ${data.date_supplier_verified ? 'disabled' : ''}> No
                </label>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${index+el.id}" value=3 ${el.value==3 ? 'checked' : ''} ${data.date_supplier_verified ? 'disabled' : ''}> N/A
                </label>  
              </td>
              <td><input type=text class=form-control name=corrective_action value='${el.corrective_action}' ${data.date_supplier_verified ? 'disabled' : ''}></td>
            </tr>`
  ...
  $(document).on('click', '#view_warehouse_supplier_modal .submit', function(){

    var modal_id = "#view_warehouse_supplier_modal"

    var formdata = []
    formdata.push(get_obj('id', $(`${modal_id} [name='id']`).val()))
    formdata.push(get_obj('date_received', $(`${modal_id} [name='date_received']`).val()))
    formdata.push(get_obj('arrival_time', $(`${modal_id} [name='arrival_time']`).val()))
    formdata.push(get_obj('invoice', $(`${modal_id} [name='invoice']`).val()))
    formdata.push(get_obj('trailer_no', $(`${modal_id} [name='trailer_no']`).val()))
    formdata.push(get_obj('trailer_plate', $(`${modal_id} [name='trailer_plate']`).val()))
    formdata.push(get_obj('trailer_seal', $(`${modal_id} [name='trailer_seal']`).val()))

    var supplier_inspected_by_name = $(`${modal_id} [name='supplier_inspected_by_name']`).val()
    if(supplier_inspected_by_name){
      supplier_inspected_by_name = supplier_inspected_by_name.trim()
      formdata.push(get_obj('supplier_inspected_by_sig', $(`${modal_id} [name='supplier_inspected_by_sig']`).val()))
      formdata.push(get_obj('supplier_inspected_by_name', supplier_inspected_by_name))
      formdata.push(get_obj('supplier_inspected_by_position', $(`${modal_id} [name='supplier_inspected_by_position']`).val()))
    }

    var supplier_verified_by_name = $(`${modal_id} [name='supplier_verified_by_name']`).val().trim()
    if(supplier_verified_by_name){
      formdata.push(get_obj('supplier_verified_by_sig', $(`${modal_id} [name='supplier_verified_by_sig']`).val()))
      formdata.push(get_obj('supplier_verified_by_name', supplier_verified_by_name))
      formdata.push(get_obj('supplier_verified_by_position', $(`${modal_id} [name='supplier_verified_by_position']`).val()))
    }

    var checklist = []
    $("#warehouse_receiving_table1 tbody tr, #warehouse_receiving_table2 tbody tr").each(function(){
      var id    = $(this).attr('id')
      var value = $(this).find('[type="radio"]:checked').val()
      var corrective_action = $(this).find('[name="corrective_action"]').val()
      if(value){
        var obj = {
          id : id,
          value : value,
    ...
    $.ajax({
      ...
      data : {
        ...
        supplier_inspected : $(`${modal_id} [name='supplier_inspected_by_name']`).val() ? true : false,
        supplier_verified  : $(`${modal_id} [name='supplier_verified_by_name']`).val() ? true : false
      },
      success: function(response){
        $(modal_id).modal("hide")
        // swal.close();
  ...
  $(document).on('click','.add_item_btn', function(){
    ...
    $.ajax({
      ...
      data: {
        add_entry              : true,
        warehouse_receiving_id : tr.find('input[name="warehouse_receiving_id"]').val(),
        item_type              : item_type,
      ...
      success: function(response){
        if(response.type = 'success'){
          var counter1 = $(`#warehouse_receiving_table${item_type} tbody tr`).length
          var div = `<tr id=${response.message}>
              <td>${counter1+1}</td>
              <td>${input.val()}</td>
              <td class=text-center colspan=3>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${counter1+input.val()}" value=1> Yes
                </label>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${counter1+input.val()}" value=2> No
                </label>
                <label class="radio-inline">
                  <input type="radio" name="inlineRadio${counter1+input.val()}" value=3> N/A
                </label>  
              </td>
  ...
  $(document).on('click', '.cell_value', function(e){
    ...
    parent.html(`<input type=${column=='location' ? 'text' : 'number'} class="form-control cell_input" data-column='${column}' value='${value=='Set' ? '' : value}'>`)
  })
  ...
  $(document).on('keypress', '.cell_input', function(event){
    ...
    if(keycode == '13'){
      ...
      var parent = $(this).closest('.cell_item')
      var tr = $(this).closest("tr")
      var formdata = $("#materials_inventory .main_table").dataTable().fnGetData(tr)
      ...
      $.ajax({
        ...
        success: function (response) {
          parent.html(`<a href="#" class="cell_value text-primary" data-column='${column}'>${value}</a>`)
          materials_inventory_table.ajax.reload()
          growl(response.message, response.type)
        },
        error: function (response) {
  ...
  $(document).on('change', '#add_stock_transfer_modal [name="location_id"]', function(){

    var modal_id = "#add_stock_transfer_modal"
    var value = $(this).val()
    if(!value){
      ['raw_material_id','current_stock','uom','quantity','transfer_for','transfer_date','notes'].forEach(el=>{
  ...
  $(document).on('change', '#add_stock_transfer_modal [name="raw_material_id"]', function(){

    var value = $(this).val()
    $("#stock_transfer_sources tbody").html('')
    if(!value){
      ['current_stock','uom','quantity','transfer_for','transfer_date','notes'].forEach(el=>{
        $(`${modal_id} [name='${el}']`).val('')
      })
      return
    }
    ...
    $.ajax({
      ...
      data: {
        ...
        location_id : $('#add_stock_transfer_modal [name="location_id"]').val()
      },
      success: function ({item, sources}) {
        
        $('#add_stock_transfer_modal [name="current_stock"]').val(item.current_stock)
        $('#add_stock_transfer_modal [name="uom"]').val(item.uom)
        $('#add_stock_transfer_modal [name="transfer_date"]').val(current_date)
  ...
  $(document).on('change', '#stock_transfer_sources tbody .source', function(){
    update_stock_quantity()
  })
  ...
  $(document).on('click', '.receive_stock_transfer', function(e){
    ...
    $.ajax({
        ...
        success: function ({details}) {
          ...
          // console.log(details)
          var elems = ["id","stock_no","lot_batch_no","location","raw_materials","current_stock","uom","quantity","transfer_for","transfer_date"]
          elems.forEach(el=>{
          ...
          if(details.date_received){
            ...
            var sig_div1 = get_fsig("Received By", details.received_by_sig, details.received_by_name, details.received_by_position, details.date_received)
  ...
  $(document).on('click', '#view_stock_transfer_modal .submit', function(){
    ...
    if(received_by_name){
      ...
      form_items.push(get_obj('id', $(`${modal_id} [name='id']`).val()))
      form_items.push(get_obj('received_by_sig', $(`${modal_id} [name='received_by_sig']`).val()))
      form_items.push(get_obj('received_by_name', received_by_name))
      form_items.push(get_obj('received_by_position', $(`${modal_id} [name='received_by_position']`).val()))
    }else{
      alert("Please provide name of receiver")
      return
    }
  ...
  $(document).on('change', '.select_location', function(){
    ...
    switch(tab_id){
      ...
      case 'stock_transfer_out':
        stock_transfer_out_table.columns(3).search(search_item).ajax.reload()    
      break;
    }
  })
  ...
  $(document).on('click', '.view_stock_card', function(e){
    ...
    var raw_material_id = row.raw_material_id
    var raw_material    = row.material_name
    var location_id     = row.location_id
    var location        = row.location

    var start_date = $(`${modal_id} [name='start_date']`).val()
    var end_date = $(`${modal_id} [name='end_date']`).val()

    view_stock_card_details(raw_material_id, location_id, start_date, end_date)

    $(`${modal_id} [name='material_name']`).val(raw_material)
    $(`${modal_id} [name='location']`).val(location)
    $(modal_id).modal("show")
  })

  $(document).on('click', '#view_stock_card .submit', function(){
  ...
  function view_stock_card_details(raw_material_id, location_id, start_date, end_date){

    if($.fn.DataTable.isDataTable("#stock_card_table")){
      $("#stock_card_table").DataTable().clear().destroy();
    }
  ...
  for(let id=1; id<=8; id++){
    ...
    $(document).on("click",'.clear-btn'+id, function(){
       $(this).siblings('#signature-pad-'+id).jSignature('clear');
       $(this).siblings('.signature-data-text'+id).val('');
    ...
    // $('#signature-pad-'+id).on('change', function(){
       ...
       $(this).siblings('.signature-data-text'+id).val(signatureData);
