                    ...
                    if($_COOKIE['employee_id'] > 0) {
                        ...
                        foreach($ai_query as $ai_orw) {
                            ...
                            if(!in_array($temp_id,$array_data2)) {
                                $getId = $ai_orw['id'];
                                // $queryOpen = "SELECT COUNT(*) as countOpen FROM tbl_meeting_minutes_action_items where action_meeting_id = $getId ";
                                $resultOpen2 = mysqli_query($conn, "SELECT COUNT(*) as countOpen FROM tbl_meeting_minutes_action_items where action_meeting_id = $getId ");
                                if ( mysqli_num_rows($resultOpen2) > 0 ) {
                                    $rowOpen2 = mysqli_fetch_array($resultOpen2);
                                    $action_item += $rowOpen2['countOpen'];
                                }
                                // while($rowOpen2 = mysqli_fetch_array($resultOpen2)) { 
                                //     $action_item += $rowOpen2['countOpen'];
                                // }
                            } 
                        }
                                        ...
                                        $array_data = explode(", ", $row["attendees"]);
                                            ...
                                            if(in_array($temp_id,$array_data)) {
                                                ...
                                                while($row_open = mysqli_fetch_array($resultOpen)) {
                                                    echo '<tr id="statusTbl_'.$row_open['action_id'].'">
                                                        <td>'.$row_open['action_details'].'</td>
                                                        <td>';
                                                        ...
                                                        echo '</td>
                                                        <td>'.date('Y-m-d', strtotime($row_open['target_due_date'])).'</td>
                                                        <td>'.$row_open['status'].'</td>
                                                        <td width="60px">
                                                            <a href="#modal_comment_ai" data-toggle="modal" type="button" id="comment_AI" data-id="'.$row_open['action_id'].'">
                                                                ...
                                                                foreach($comment_count as  $ccout) { 
                                                                    $color= 'red';
                                                                    if($ccout['count'] != 0){ $color= 'blue'; }

                                                                    echo '<span class="badge" style="background-color:'.$color.'; margin-left:-7px;"><b style="font-size:12px;">'.$ccout['count'].'</b></span>';

                                                                }
                                                            echo '</a>
                                                        </td>
                                                        <td><a href="#modal_update_status" data-toggle="modal" class="btn red btn-xs" type="button" id="add_status" data-id="'.$row_open['action_id'].'" >Update</a></td>
                                                    </tr>';
                                                }
                                            }
                                        }
