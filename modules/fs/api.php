...
if (isset($_GET['add-haccp-team-roster'])) {
    try {
        ...
        $conn->execute("INSERT INTO tbl_fsp_team_roster($columns) VALUE($params)", $values);
        $conn->commit();
...
if (isset($_GET['update-haccp-team-roster'])) {
    try {
        ...
        $conn->execute("UPDATE tbl_fsp_team_roster SET $paramset WHERE id = ?", [...$values, $roster]);
        $conn->commit();
...
if (isset($_GET['remove-haccp-team-roster'])) {
    try {
        ...
        $conn->execute("UPDATE tbl_fsp_team_roster SET deleted_at = ? WHERE id = ?", $currentTimestamp, $roster);
        $conn->commit();
...
if (isset($_GET['update-organization-chart'])) {
    try {
        ...
        $existingRecord = $conn->execute('SELECT * FROM tbl_fsp_org_charts WHERE user_id = ?', $user_id)->fetchAssoc();

        if (count($existingRecord)) {
            $conn->execute("UPDATE tbl_fsp_org_charts SET image = ?, portal_user=? WHERE user_id = ?", $newChart, $portal_user, $user_id);
        } else {
            $conn->execute("INSERT INTO tbl_fsp_org_charts(user_id, portal_user, image) VALUE(?,?,?)", $user_id, $portal_user, $newChart);
        }
...
if (isset($_GET['remove-organization-chart'])) {
    try {
        $conn->execute("DELETE FROM tbl_fsp_org_charts WHERE user_id = ?", $user_id);
