tcTable.prototype.ccp_determination = function () {
    ...
    processes.forEach((id) => {
        ...
        // row variables
        let q1, q2, q3, q4, ccpIndicator, ccpCell, updCCPInput;
        let i = 0;

        Object.keys(ccp).forEach((k) => {
            if (k == "ccpNumber") return; // skip

            const rowAttr = { "data-id": id, "data-hazard": k };

            // first item, flag as main row
            i == 0 && (rowAttr["data-main-row"] = "");

            // creating row element
            rows[k] = new HRow(table, rowAttr);

            // render first row (with rowspan)
            i == 0 &&
            ...
            rows[k].cell(k, { class: "bold" }, { textAlign: "center" });
            rows[k].cell(ha[k].potentialHazards, null, { width: "16%" });
            q1 = rows[k].cell(null, { "data-question": "q1" });
            q2 = rows[k].cell(null, { "data-question": "q2" });
            q3 = rows[k].cell(null, { "data-question": "q3" });
            q4 = rows[k].cell(null, { "data-question": "q4" });

            if (i == 0) {
                ccpCell = rows[k].cell(null, {
                    rowspan: 6,
                    class: "text-center",
                });
                // rendering ccp indicator column (first row only)
                ccpIndicator = ccpCell.append(
                    `<div style="text-align: center; margin: 0 0 1rem 0;" data-ccpindicator>${
                        ccp.ccpNumber || ""
                    }</div>`
                );

                // keep tracking ccp conditions for setting ccp number column
                updCCPInput = () => {
                    let isCCP = false;
                    ...
                    const ccpId = DiagramObject.markCCP(id, isCCP).filter(
                        (x) => id == x
                    )[0];
                    if (isCCP) {
                        // task: identify what ccp number is  this
                        const ccpNumber =
                            planBuilder.processes[ccpId].ccpDetermination
                                .ccpNumber;
            ...
            qChain(q1, q2, q3, q4, updCCPInput, ccp[k]);
            i++;
        });
        ...
        // const rowA = new HRow(table, { "data-id": id, "data-hazard": "A" });
        // rowA.cell("P", { class: "bold noborder" }, { textAlign: "center" });
        // rowA.cell(ha.P.potentialHazards, null, { width: "16%" });
        // q1 = rowA.cell(null, { "data-question": "q1" });
        // q2 = rowA.cell(null, { "data-question": "q2" });
        // q3 = rowA.cell(null, { "data-question": "q3" });
        // q4 = rowA.cell(null, { "data-question": "q4" });
        // qChain(q1, q2, q3, q4, updCCPInput, ccp.P);
    });
...
function qChain(q1, q2, q3, q4, updateCCP, ccp) {
    ...
    const q1a = q1.on("yesno", (v) => {
        if (v == "n") {
            ...
            (ccp.q2 = ""),
                ...
                dispatchAnswer(q2yn, null, true);
            (ccp.q3 = ""),
                (q3yn.style.display = "none"),
                dispatchAnswer(q3yn, null, true);
            (ccp.q4 = ""),
                (q4yn.style.display = "none"),
                dispatchAnswer(q4yn, null, true);
        } else if (v == "y") {
            ccp.q1 = "y";
    ...
    const ft = q1.append(
        `<div style="text-align: center; margin: 0 0 1rem 0;">Is control at this step necessary for safety?</div>`
    );
    const q1b = q1.on("yesno", (v) => {
        if (v == "n") {
            ...
            // yes, return to q1a
            rt.innerText =
                "Modify step, process or product... return to Question 1.";
            // reset question 1
            // expires 3 seconds
            displayControlHazard(false);
    ...
    const rt = q1.append(
        `<div class="text-muted italic text-center" style="margin: 0 0 1rem 0;"></div>"`
    );
    const cq = q1.append(
        `<div class="text-center" style='margin-bottom: 1rem;'>Identify how and where this hazard will be controlled.</div>"`
    );
    const ci = q1.on(
        "input",
        `<div style="padding: var(--padBordered);">${ccp.control || ""}</div>`,
        (v) => {
            ccp.control = v;
        }
    );
...
function dispatchAnswer(el, value, simulate = false) {
    let radio;
    (radio = el.querySelector("input[type=radio]:checked")) &&
        (radio.checked = false);

    if ("y" == value || "n" == value) {
        (radio = el.querySelector(`input[type=radio][value=${value}]`)) &&
            (radio.checked = true);
    }

    if (simulate) {
        el.querySelectorAll("input[type=radio]:checked").forEach((e) =>
