<?php
        ...
        try {
            ...
            switch($pdfView) {
                case 'importer_information': {
                    // Fetch base data with necessary joins in one query
                    $data = $conn->execute("
                        SELECT 
                            i.id, 
                            i.date_signed, 
                            ...
                            (
                                SELECT GROUP_CONCAT(mat.material_name SEPARATOR ', ') 
                        ...
                        WHERE MD5(i.id) = ?
                    ", $user_id, $recordId)->fetchAssoc();
                
                    if (!$data) {
                        throw new Exception("Importer information not found.");
                    }
                
                    // Format address after fetching data
                    $data['importer_address'] = formatSupplierAddress($data['importer_address']);
                
                    // debugOutput($data, DEBUG);
                ...
                case 'evaluation_form': {
                    ...
                    // debugOutput($data['products'], DEBUG);
                ...
                case 'ipr': {   
                    $data = $conn->execute("SELECT 
                            ...
                            iby2.intended_use,
                            iby2.reviewed_by,
                            iby2.reviewed_by_sign,
                            iby2.review_date,
                            iby2.approved_by,
                            iby2.approved_by_sign,
                            iby2.approve_date,
                            iby2.comments,
                    ...
                    $mpdf->SetTitle($title);
                    $mpdf->AddPage();
                    $mpdf->writeHTML($css . getLayout('ipr-1', $title, $data));
                    $mpdf->writeHTML(getLayout('ipr-2', $title, $data));
                    $mpdf->writeHTML($css . getLayout('ipr-3', $title, $data));
    ...
    function debugOutput($data, $mode = false) {
        global $debugging;
                ...
                <br><table>
                    <tr>
                        <td>Prepared By:<br />
                         ...
                         <td>Date:
                            <p style="text-align:center;">'.txt($data['prepare_date']).'</p>
                        </td>
                    </tr>
                    <tr>
                      <td>Reviewed By:<br />
