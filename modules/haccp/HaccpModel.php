...
class Haccp {
    ...
    public function setId($id, $hashed = false) {
        ...
        try {
            ...
            $data = $this->conn->execute("SELECT id, title, user_id,
                (SELECT id FROM tbl_haccp_version WHERE haccp_id = h.id AND since IS NOT NULL ORDER BY created_at DESC LIMIT 1) AS vid,
                (SELECT facility_id FROM tbl_haccp_version_details WHERE version_id = vid) AS facility,
                (SELECT id FROM tbl_haccp_version_details WHERE version_id = vid) AS did
                FROM tbl_haccp h WHERE ". ($hashed ? 'md5(id)' : 'id') ." = ? AND deleted_at IS NULL", $id
            )->fetchAssoc();
    ...
    public function setVersion($version_id, $hashed = false) {
        ...
        try {
            ...
            $data = $this->conn->execute("SELECT id, title, user_id,
                (SELECT id FROM tbl_haccp_version WHERE ". ($hashed ? 'MD5(id)' : 'id') ." = ?) AS vid,
                (SELECT facility_id FROM tbl_haccp_version_details WHERE version_id = vid) AS facility,
                (SELECT id FROM tbl_haccp_version_details WHERE version_id = vid) AS did
                FROM tbl_haccp h WHERE id = ? AND deleted_at IS NULL", $version_id, $this->id
            )->fetchAssoc();
    ...
    public function getResource($withComponents = false) {
        $details = $this->conn->execute("SELECT * FROM tbl_haccp_version_details WHERE id = ?", $this->details_id)->fetchAssoc();
        $components = $this->conn->execute("SELECT * FROM tbl_haccp_components WHERE version_id = ?", $this->version_id)->fetchAll();
    ...
    public function getDiagramImage() {
        $details = $this->conn->execute("SELECT diagram_image FROM tbl_haccp_version_details WHERE id = ?", $this->details_id)->fetchAssoc();
    ...
    public function versionStatus() {
        $version = $this->conn->execute("SELECT status FROM tbl_haccp_version WHERE id = ?", $this->version_id)->fetchAssoc();
        return $this->getEqStatus($version['status']);
    }
    
    public function getEqStatus($status) {
        switch($status) {
            case self::$step['1']: return 'Draft';
            case self::$step['2']: return 'For Review';
            case self::$step['3a']: return 'For Approval';
            case self::$step['3b']: return 'For Revision';
            case self::$step['4a']: return 'Approved by CIG';
            case self::$step['4b']: return 'Returned To Reviewer';
            case self::$step['4c']: return 'Returned To Drafter';
            case self::$step['5']: return 'Reviewed by Client';
            case self::$step['6']: return 'Approved by Client';
    ...
    public function getLogs($user_id, $version = null) {
        ...
        $logs = $this->conn->execute("SELECT l.*, CONCAT(hr.first_name, ' ', hr.last_name) AS logger, hr.user_id AS employer_id FROM tbl_haccp_logs l LEFT JOIN tbl_user u ON u.id = l.portal_user JOIN tbl_hr_employee hr ON u.employee_id = hr.ID WHERE MD5(version_id) = ? ORDER BY l.created_at DESC", $version)->fetchAll();
        $data = [];
    ...
    public function getChangeHistory($version = null) {
        ...
        $data = $this->conn->execute("SELECT * FROM tbl_haccp_version WHERE id = ?", $version)->fetchAssoc();
        if(count($data)) {
            $lastLog = $this->conn->execute("SELECT description FROM tbl_haccp_logs WHERE version_id = ? ORDER BY created_at DESC LIMIT 1", $data['id'])->fetchAssoc();
            $firstLog = $this->conn->execute("SELECT portal_user FROM tbl_haccp_logs WHERE version_id = ? ORDER BY created_at ASC LIMIT 1", $data['id'])->fetchAssoc();
    ...
    public function create($arr) {
        try {
            ...
            $this->conn->execute("INSERT INTO tbl_haccp({$haccpParams['columns']}) VALUE({$haccpParams['params']})", $haccpParams['values']);
    ...
    public function update($arr, $user, IIQ_User $currentUser) {
        try {
            ...
            // update haccp title
            $this->conn->execute("UPDATE tbl_haccp SET title = ? WHERE id = ?", $arr['description'], $this->id);
            ...
            switch($arr['save_as'] ?? null) {
                ...
                default: 
                    ...
                    $this->conn->execute("UPDATE tbl_haccp_version SET supersedes = ?, since = NULL WHERE id = ?", $prevVersionId, $this->version_id);
    ...
    public function newVersion($status, $data) {
        ...
        $this->conn->execute("INSERT INTO tbl_haccp_version (haccp_id, status) VALUE(?,?)", $this->id, $status);
        ...
        $this->conn->execute("INSERT INTO tbl_haccp_version_details ({$d['columns']}) VALUE({$d['params']})", $d['values']);        
    ...
    public function updateVersionStatus($status) {
        $this->conn->execute("UPDATE tbl_haccp_version SET status = ? WHERE id = ?", $status, $this->version_id);
        return true;
    }
    ...
    public function updateCurrentVersion($data) {
        ...
        $this->conn->execute("UPDATE tbl_haccp_version_details SET {$d['params_set']} WHERE version_id = ?", [...$d['values'], $this->version_id]);                
    ...
    public function haccpComponents($data) {
        ...
        $this->conn->execute("DELETE FROM tbl_haccp_components WHERE version_id = ?", $this->version_id);

        // replace with new data
        $params = implode(',', array_map(function($x) { return "($x)"; }, $params));
        $this->conn->execute("INSERT INTO tbl_haccp_components ($columns) VALUES $params", $values);        
    }

    public function updateDiagramImage($image) {
        $this->conn->execute("UPDATE tbl_haccp_version_details SET diagram_image = ? WHERE id = ?", $image, $this->details_id);
        return true;
    }
    ...
    public function createLog($description, $category, $portal_user) {
        ...
        $this->conn->execute("INSERT INTO tbl_haccp_logs ({$params['columns']}) VALUE( {$params['params']} )", $params['values']);
        return $this->conn->lastInsertID();
    }

    public function newAction($params) {
        $this->conn->execute("INSERT INTO tbl_haccp_actions ({$params['columns']}) VALUE({$params['params']})", $params['values']);
        $id = $this->conn->lastInsertID();
        return $id;
    }

    public function updateAction($params, $id) {
        $this->conn->execute("UPDATE tbl_haccp_actions SET {$params['params_set']} WHERE id = ?", $params['values'], $id);
    }
    ...
    public function getAction($hashId = null) {
        ...
        if($hashId == null) {
            ...
            $action = $this->conn->execute("SELECT * FROM tbl_haccp_actions WHERE version_id = ? AND status = ? ORDER BY created_at DESC LIMIT 1", $this->version_id, 'open')->fetchAssoc();
        } else {
            // session id or hashid is provided, check directly
            $action = $this->conn->execute("SELECT * FROM tbl_haccp_actions WHERE MD5(id) = ?", $hashId)->fetchAssoc();
        }
        return (count($action)) ? $action : null;
    }

    public function teamStructure() {
        $team = $this->conn->execute("SELECT * FROM tbl_haccp_team_roster WHERE user_id=? AND deleted_at IS NULL", $this->owner_id)->fetchAll();
        $teamRoster = [];
    ...
    public function getSigns() {
        ...
        foreach($team as $id => $t) {
            $data = $this->conn->execute("SELECT * FROM tbl_haccp_pfd_signs WHERE signee_id = ? AND version_id = ?", $id, $this->version_id)->fetchAssoc();
            $struc[] = array(
                'id' => $data['id'] ?? '',
                'signee_id' => $id,
                'name' => $t['name'],
        ...
        try {
            ...
            foreach($data as $k => $d) {
                ...
                if(isset($d['id'])) {
                    ...
                    $this->conn->execute("UPDATE tbl_haccp_pfd_signs SET {$p['params_set']} WHERE id = ?", [...$p['values'], $d['id']]);
                } else {
                    $params['version_id'] = $this->version_id;
                    $params['signee_id'] = $d['account'];
                    $p = self::paramBuilder($params);
                    $this->conn->execute("INSERT INTO tbl_haccp_pfd_signs ({$p['columns']}) VALUE({$p['params']})", $p['values']);
                }
            }
    ...
    public function addTask($data, $portal_user) {
        try {
            ...
            $this->conn->execute("INSERT INTO tbl_haccp_tasks ({$params['columns']}) VALUE({$params['params']})", $params['values']);
            $id = $this->conn->lastInsertID();
            $data = $this->conn->execute("SELECT *, (SELECT CONCAT(first_name, ' ', last_name) as assignee FROM tbl_hr_employee WHERE t.assignee_id IS NOT NULL AND ID = t.assignee_id) AS assignee_name FROM tbl_haccp_tasks t WHERE id = ?", $id)->fetchAssoc();
    ...
    public function getAllTasks() {
        $data = $this->conn->execute("SELECT *, (SELECT CONCAT(first_name, ' ', last_name) as assignee FROM tbl_hr_employee WHERE t.assignee_id IS NOT NULL AND ID = t.assignee_id) AS assignee_name FROM tbl_haccp_tasks t WHERE version_id = ?", $this->version_id)->fetchAll();
        return $data;
    }

    public function getPostDocSigns($all = false) {
        $data = $this->conn->execute("SELECT developed_by, developed_at, developer_sign, reviewed_by, reviewed_at, reviewer_sign, approved_by, approved_at, approver_sign FROM tbl_haccp WHERE id = ?", $this->id)->fetchAssoc();
    ...
    public function updatePostSigns($arr) {
        try {
            ...
            if(!$this->conn->execute("UPDATE tbl_haccp SET {$q['params_set']} WHERE id = ?", [...$q['values'], $this->id])) {
    ...
    public static function updateTaskStatus($taskId, $status) {
        global $conn;
        try {
            $conn->execute("UPDATE tbl_haccp_tasks SET status = ? WHERE id = ?", $status, $taskId);
    ...
    public static function paramBuilder($arr) {
        ...
        return [
            ...
            'params' => implode(',', array_map(function($a) { return '?'; }, $paramKeys)),
            'params_set' => implode(',', array_map(function($a) { return $a .'=?'; }, $paramKeys)),
            'values' => $paramValues,
        ];
    }
    ...
    public static function all($owner) {
        global $conn;
        $records = $conn->execute("SELECT id FROM tbl_haccp WHERE user_id = ? AND deleted_at IS NULL", $owner)->fetchAll();
