...
function fetchHaccpTeamRoster($conn, $user_id) {
    $team = $conn->execute("SELECT * FROM tbl_haccp_team_roster WHERE user_id=? AND deleted_at IS NULL", $user_id)->fetchAll();
    $teamRoster = [];
...
function isYes($v) { return $v == 'y'; }
function isNo($v) { return $v == 'n'; }
