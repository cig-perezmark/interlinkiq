...
<style type="text/css">
    ...
    <div class="row">
        ...
        <script type="text/javascript">
                ...
                $.ajax({
                    ...
                    success: function(data){
            		    ...
            		    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                            ...
                            const date = data[4]; // Adjust index to match your "Due Date" column
                        
                            if (!min && !max) return true;
                            
                            const rowDate = new Date(convertToISO(date));
                            const minDate = min ? new Date(convertToISO(min)) : null;
                            const maxDate = max ? new Date(convertToISO(max)) : null;
            ...
            // File Section
            function get_myTask2(users_id,h_id) {
                // alert(h_id);
                $.ajax({
                    type: "GET",
            ...
            function get_myTask(users_id,h_id) {
                // alert(id);
                var newwParent = $('#data_child'+h_id+' > tr');

                $("#hide_box tr").empty();
                $("#main_task > tr").empty();
                $('html,body').animate({scrollTop:$("#center_"+h_id).offset().top}, 'slow');
                // if (newwParent.length == 0) {
                        // url:'mypro_function/fetch_task_function.php',
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            if (h_id == h_id)
                            $('#data_child'+h_id).empty();
                            $('#data_child'+h_id).append(response);
                            $('html,body').animate({scrollTop:$("#center_"+h_id).offset().top}, 'slow');
                            $('#'+h_id).addClass('in');
                            $('#'+h_id).unblock();
                        }
