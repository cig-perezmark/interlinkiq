                    ...
                    <div class="tab-content">
                        <form action="controller.php" method="POST" enctype="multipart/form-data">
                            <div id="forms" class="tab-pane active">
                                                                    ...
                                                                    $equipment_result = mysqli_query($pmp_connection, $select_equipment);
                                                                     $equipment_row = mysqli_fetch_assoc($equipment_result);
                                                                        ...
                                                                        <div class="col-md-3 item-list">
                                                                            <div class="item">
                                                                                <span class="item-text"><?php echo $equipment_row['equipment_name'] ?></span>
                                    ...
                                    <div class="row" style="margin-top:15px">
                                        <?php
                                            ...
                                            foreach($result as $row):
                                                $category_id = $row['id'];
                                                $select_sub = "SELECT * FROM parameters INNER JOIN new_emp_default ON new_emp_default.parameter_cateogry_id =  parameters.parameter_id WHERE new_emp_default.parameter_id = $category_id AND zone_id = $zone_id";
                                                $sub_result = mysqli_query($emp_connection,$select_sub);
                                        ?>
                                        <div class="col-md-3">
                                            <label class="caption-subject font-dark bold uppercase"><?= $row['category_name'] ?></label><br>
                                            <?php foreach($sub_result as $sub_row): 
                                                // Sample usage
                                                $minimum_value = $sub_row['minimum_value'];
                                                $maximum_value = $sub_row['maximum_value'];
                                            ...
                                            ?>
                                                <label style="margin-top:10px"><?= $sub_row['Parameter'] ?></label>
                                                <div class="d-flex">
                                                    <input type="text" class="form-control result-input input-field" data-min="<?= $minimum_value ?>" data-max="<?= $maximum_value ?>" placeholder="Enter result">
                                                    <input type="text" class="form-control" value="<?= $sub_row['oum']?>" style="width:120px" readonly>
                                                </div>
                                            <?php endforeach ?>
                                        </div>
                                        <?php endforeach; ?>
                                    ...
                                    <div class="row" style="margin-top:15px">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="datetime-local" name="recevied_date" class="form-control">
                                        </div>
                                    </div>
                                    <div class="row" style="margin-top:15px">
                                        <div class="col-md-12">
                                            <button class="btn btn-primary" type="submit" name="save_new_emps">Submit</button>
                                            <hr>
                                        </div>
                                    </div>
                            </div>
                            ...
                            <div id="records" class="tab-pane">
                                <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1_2" style="margin-top:15px">
                                    <thead class="bg-primary">
                                        <tr>
                                            <td>Equipment/Area  Collection Descriptio</td>
                                            <td>Image Of Sampling Point</td>
                                            <td>Type: (Sruface / Scrappings / Water / Air)</td>
                                            <td>Sampled By:</td>
                                            <td>Sampled Date/Time</td>
                                            <td>Action</td>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php 
                                            $query = "SELECT * FROM new_emps WHERE zone_id = $zone_id";
                                            $result = mysqli_query($emp_connection, $query);
                                            foreach ($result as $row): 
                                        ?>
                                            <tr>
                                                <td><?= $row['equipment_area'] ?></td>
                                                <td><img src="uploads/emp/<?= $row['image_sampling'] ?>" alt="Image" style="max-width: 100px;"></td>
                                                <td><?= $row['type'] ?></td>
                                                <td><?= $row['sampled_by'] ?></td>
                                                <td><?= $row['sampled_date'] ?></td>
                                                <td><a href="new_emps_area.php?emp_id=<?= $row['id'] ?>" target="_blank" class="btn btn-primary">View</a></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
