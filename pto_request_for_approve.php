                        ...
                        <div id="forms" class="tab-pane active">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                   ...
                                   $sql = "SELECT * FROM leave_details INNER JOIN tbl_user ON tbl_user.ID = leave_details.payeeid INNER JOIN leave_types ON leave_types.leave_id = leave_details.leave_id WHERE leave_details.approve_status = 0 AND leave_details.payeeid = '$explode_values'" ; 
                                    $result = mysqli_query ($conn, $sql);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a id="approve_pto" leave_count="<?= $rows['leave_count'] ?>" leave_ids="<?= $rows['ids'] ?>" minute="<?= $rows['leave_id'] ?>" task_date="<?= $rows['start_date'] ?>" user_id="<?= $rows['payeeid'] ?>" class="btn btn-outline blue edit">Approve</a>
                                            <a id="disapproved_pto" data-toggle="modal" data-target="#exampleModalCenter" leave_count="<?= $rows['leave_count'] ?>" leave_ids="<?= $rows['ids'] ?>" minute="<?= $rows['leave_id'] ?>" task_date="<?= $rows['start_date'] ?>" user_id="<?= $rows['payeeid'] ?>" class="btn btn-outline red">Disapprove</a>
                                        </td>
                                    </tr>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Disapproved PTO</h5>
      ...
      <div class="modal-body">
        ...
        <label>Reason</label>
        <textarea class="form-control" name="approve_notes" style="height:70px"></textarea>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" name="disapprove_pto" class="btn btn-primary">Update</button>
      </div>
    </div>
  </div>
</div>
...
$(document).ready(function(){
 ...
 $('[id*="disapproved_pto"]').click(function(){
        var user_id = $(this).attr('user_id');
        var leave_ids = $(this).attr('leave_ids');
