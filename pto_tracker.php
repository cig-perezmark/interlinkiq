                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <li>
                                <select  id="mySelect" class="form-control mb-3">
                                    <?php
                                        $select_user = "SELECT *,tbl_user.ID as user_id FROM tbl_user INNER JOIN tbl_hr_employee ON tbl_hr_employee.ID = tbl_user.employee_id WHERE tbl_hr_employee.user_id = 34 AND tbl_hr_employee.status = 1 ORDER BY tbl_user.first_name ASC";
                                        $user_result = mysqli_query($conn,$select_user);
                                        foreach($user_result as $user_rows):
                                    ?>
                                        <option value="<?= $user_rows['user_id'] ?>"><?= $user_rows['first_name'].' '.$user_rows['last_name'] ?></option>
                                    <?php endforeach ?>
                                </select>
                            </li>
                    ...
                    <div class="portlet-body">
                        ...
                        <div id="forms" class="tab-pane active">
                            <table class="table table-bordered table-hover" id="sample_1_2">
                                ...
                                <tbody>
                                    <?php
                                    $get_pto = mysqli_query($conn,"SELECT * FROM tbl_user INNER JOIN leave_details ON leave_details.payeeid = tbl_user.ID ORDER BY id DESC");
...
,
    ...
    $('#mySelect').change(function() {
        var optionValue = $(this).val(); // Get the selected option value
        //alert(optionValue);
        //Make an AJAX request to fetch data from the server
        $.ajax({
          url: 'controller.php', // Replace with your PHP file that fetches data
          type: 'GET',
          data: {
            action:"pto_tracker",
            option: optionValue 
